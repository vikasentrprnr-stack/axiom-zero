...
import { 
  ...
  MoreVertical, Share2, Download, CheckCircle, User, Laptop
} from 'lucide-react';
...
const ProcessingIndicator = ({ status }: { status: string }) => (
  <motion.div 
    ...
    className="flex items-center space-x-3 text-zinc-300 bg-zinc-800/50 px-4 py-2.5 rounded-full w-fit mb-2 shadow-sm border border-white/10 backdrop-blur-md"
  >
...
// PROPERLY EXTRACTED CODE BLOCK COMPONENT TO FIX REACT HOOK ERRORS
const CodeBlock = ({ node, inline, className, children, ...props }: any) => {
    ...
    return (
      <div className="my-5 rounded-xl overflow-hidden border border-white/10 bg-[#0e0e0e] shadow-2xl relative group">
        <div className="flex items-center justify-between px-4 py-2.5 bg-[#161616] border-b border-white/5">
          ...
          <button 
            onClick={handleCopy} 
            className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 hover:bg-white/10 rounded-md flex items-center space-x-1.5 text-zinc-400 hover:text-white"
          >
  ...
  return (
    <code 
      {...props} 
      className="bg-zinc-800/80 text-blue-300 px-1.5 py-0.5 rounded-md text-[13.5px] font-mono border border-white/10 break-words"
    >
      {safeCodeText}
    </code>
  );
...
export default function Home() {
  ...
  }, [messages, streamingContent, isTyping, typingStatus]);
  ...
  useEffect(() => {
    ...
    const checkEnvironment = async () => {
      ...
      if (mobile || /^((?!chrome|android).)*safari/i.test(userAgent) || !navigator.gpu) { 
      ...
      try {
        ...
        if (adapter && ((navigator as any).deviceMemory > 8 || adapter.limits.maxBufferSize >= 2147483648)) { 
          tier = 'high'; 
        }
      } catch (e) {}
      ...
      if (lastDocId) {
         ...
         if (targetDoc) {
           await handleLoadDocument(targetDoc, savedName);
         } else {
           setAppState('ready');
         }
      } else {
  ...
  const toggleSidebar = () => { 
    setIsSidebarOpen(prev => { 
      localStorage.setItem('axiom_sidebar_closed', (!prev).toString()); 
      return !prev; 
    }); 
  };
    ...
    try {
      ...
      const newEngine = await CreateWebWorkerMLCEngine(worker, AI_MODELS[modelKey].id, { 
          initProgressCallback: (progress) => {
             ...
             if (rawPct > 5 && rawPct < 100 && downloadStartTimeRef.current) {
                 ...
                 if (remaining > 60) {
                   setDownloadTimeLeft(`${Math.floor(remaining / 60)}M ${Math.floor(remaining % 60)}S`);
                 } else {
                   setDownloadTimeLeft(`${Math.floor(remaining)}S`);
                 }
             } else if (rawPct === 100) {
               setDownloadTimeLeft('0S');
             }
          }
      }, { 
      ...
      if (!isSilentLoad) {
         ...
         if (lastDocId) {
             ...
             const targetDoc = docs.find(d => d.id === lastDocId);
             if (targetDoc) { 
               await handleLoadDocument(targetDoc, userName); 
               return; 
             }
         }
         setAppState('ready');
          ...
          try { 
            caches.keys().then((cacheNames) => { 
              cacheNames.forEach((name) => { 
                if (name.includes('webllm') || name.includes('tvmjs')) caches.delete(name); 
              }); 
            }); 
          } catch(e) {}
      } else { 
        alert(`Model Initialization Failed: ${err.message}`); 
      }
      setAppState('selecting');
    }
  }, [userName]); 
      ...
      if (type === 'QUERY_EMBEDDED') {
          ...
          const finalChunks = denseResults.map(doc => {
             ...
             queryTerms.forEach(term => { 
               if (textLower.match(new RegExp(`\\b${term}\\b`, 'g'))) lexicalScore += 0.05; 
             });
          ...
          if (contextText.length > 3500) { 
            contextText = contextText.substring(0, 3500) + "... [Truncated]"; 
          }
          ...
          try {
            ...
            setMessages(prev => [
              ...prev, 
              { id: `ai-${Date.now()}`, role: 'assistant', content: fullResponse, sources: finalChunks } as ChatMessage
            ]);
          } catch (err: any) {
             if (err.name !== 'AbortError') {
               setMessages(prev => [
                 ...prev, 
                 { id: `err`, role: 'assistant', content: `System Error: ${err.message}` } as ChatMessage
               ]);
             }
          } finally {
             setStreamingContent(''); 
  ...
  const handleLoadDocument = async (doc: any, name: string) => {
    ...
    try {
        ...
        if (doc.messages && doc.messages.length > 0) {
          ...
          setMessages([
            { id: `w-${Date.now()}`, role: 'assistant', content: `Hi **${name}**, how can I help you with **${doc.name}** today?` } as ChatMessage
          ]);
        }
    } catch (e) { 
      alert("Failed to render document from cache."); 
    }
  };
  ...
  const handleIngestionComplete = async (chunks: TextChunk[], file: File) => {
    ...
    if (chunks.length > 100) {
      console.warn("PDF too large. Truncating to first 100 chunks for memory safety.");
    }
    ...
    try {
      ...
      setCurrentDocName(safeChunks[0].documentName);
      ...
      setMessages([
        { id: `w-${Date.now()}`, role: 'assistant', content: `Hi **${userName}**, I have vectorized **${safeChunks[0].documentName}**. How can I help?` } as ChatMessage
      ]);
    } catch (e) { 
      alert("PDF Rendering Failed."); 
      setAppState('ready'); 
    }
  };
  ...
  const copyToClipboard = (text: string, id: string) => { 
    navigator.clipboard.writeText(text.replace(/<think>[\s\S]*?(?:<\/think>|$)/g, '').trim()); 
    setCopiedId(id); 
  ...
  const shareDocument = (docId: string) => {
      ...
      const minifiedChat = docToShare.messages.map((m: any) => ({ 
        r: m.role, 
        c: m.content.replace(/<think>[\s\S]*?(?:<\/think>|$)/g, '').trim() 
      }));
      
      const payload = JSON.stringify({ 
        title: docToShare.name, 
        author: userName, 
        chat: minifiedChat 
      });
  ...
  const exportDocument = async (doc: any) => {
      ...
      let htmlContent = `
        <div style="border-bottom:2px solid #e5e7eb;padding-bottom:24px;margin-bottom:32px;">
          <h1 style="font-size:32px;font-weight:700;color:#000;margin:0;">Axiom-Zero Report</h1>
          <p style="color:#4b5563;font-size:16px;margin-top:12px;font-weight:500;">Source Document: ${doc.name}</p>
          <p style="color:#9ca3af;font-size:12px;margin-top:6px;">Generated for <strong>${userName}</strong> on ${new Date().toLocaleDateString()}</p>
        </div>
      `;
      ...
      doc.messages.forEach((m: any) => {
          ...
          htmlContent += `
            <div style="margin-bottom:28px;padding:24px;border-radius:16px;background-color:${isUser ? '#f9fafb' : '#f0fdf4'};border:1px solid ${isUser ? '#f3f4f6' : '#dcfce7'};page-break-inside:avoid;">
              <div style="font-weight:600;color:${isUser ? '#374151' : '#166534'};margin-bottom:12px;font-size:14px;text-transform:uppercase;letter-spacing:0.05em;">
                ${isUser ? userName : '🧠 Axiom-Zero'}
              </div>
              <div style="line-height:1.7;font-size:14px;color:#1f2937;">
                ${cleanContent}
              </div>
            </div>
          `;
      ...
      html2pdf().set({ 
        margin: 0, 
        filename: `Axiom_Report_${cleanFileName}.pdf`, 
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    const handleMouseMove = (ev: MouseEvent) => {
       ...
       animId = requestAnimationFrame(() => { 
         setLeftWidth(Math.max(25, Math.min((ev.clientX / window.innerWidth) * 100, 60))); 
       });
    };
  ...
  // RENDERING GUARDS: ORDERED CORRECTLY
  if (isMobile) {
    return (
      <div className="flex h-screen w-screen bg-[#050505] text-white items-center justify-center p-6 font-sans selection:bg-blue-500/30">
        <motion.div 
          initial={{ opacity: 0, y: 20 }} 
          animate={{ opacity: 1, y: 0 }} 
          transition={{ duration: 0.6, ease: smoothEase }} 
          className="max-w-md w-full p-8 bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-2xl text-center relative overflow-hidden"
        >
           ...
           <h2 className="text-2xl font-semibold mb-3 tracking-tight">Desktop Architecture Required</h2>
           <p className="text-sm text-zinc-400 leading-relaxed mb-8">
              Axiom-Zero utilizes advanced WebGPU APIs to run multi-billion parameter neural networks directly on local silicon. Mobile architectures currently restrict the VRAM allocation necessary for safe inference.
           </p>
           <div className="p-4 bg-white/5 rounded-xl border border-white/5 text-left space-y-3">
              <h4 className="text-xs font-semibold text-zinc-300 uppercase tracking-wider">How to Access</h4>
              <ul className="space-y-2 text-sm text-zinc-500">
                 <li className="flex items-start">
                   <Laptop className="w-4 h-4 mr-2 text-zinc-400 shrink-0 mt-0.5" /> 
                   Open this exact URL on a PC or Mac.
                 </li>
                 <li className="flex items-start">
                   <CheckCircle className="w-4 h-4 mr-2 text-emerald-400 shrink-0 mt-0.5" /> 
                   Recommended: 8GB+ RAM system.
                 </li>
              </ul>
           </div>
        </motion.div>
    ...
    return (
      <div className="flex h-screen w-screen bg-[#050505] text-white items-center justify-center p-6 font-sans selection:bg-blue-500/30">
        ...
        >
           ...
           <h2 className="text-2xl font-semibold mb-3 tracking-tight">Unsupported Browser Engine</h2>
           <p className="text-sm text-zinc-400 leading-relaxed mb-8">
              Your current browser does not fully support the WebGPU API required for local tensor computations. Apple Safari restricts WebGPU on certain versions.
           </p>
           <div className="p-4 bg-white/5 rounded-xl border border-white/5 text-left space-y-3">
              <h4 className="text-xs font-semibold text-zinc-300 uppercase tracking-wider">Supported Browsers</h4>
              <ul className="space-y-2 text-sm text-zinc-500">
                 <li className="flex items-center">
                   <CheckCircle className="w-4 h-4 mr-2 text-emerald-400 shrink-0" /> Google Chrome (Recommended)
                 </li>
                 <li className="flex items-center">
                   <CheckCircle className="w-4 h-4 mr-2 text-emerald-400 shrink-0" /> Microsoft Edge
                 </li>
                 <li className="flex items-center">
                   <CheckCircle className="w-4 h-4 mr-2 text-emerald-400 shrink-0" /> Brave Browser
                 </li>
              </ul>
           </div>
        </motion.div>
  ...
  return (
    <div className="flex h-screen w-screen bg-[#050505] text-white font-open-sans overflow-hidden">
        ...
        {appState === 'ask_name' && (
          ...
          >
             <div className="max-w-md w-full text-center p-6">
                ...
                >
                   <div className="w-16 h-16 rounded-3xl bg-white/5 border border-white/10 flex items-center justify-center mx-auto mb-8 shadow-2xl">
                   ...
                   <form onSubmit={(e) => { 
                     ...
                     if(nameInput.trim()){ 
                       ...
                       localStorage.setItem('axiom_user_name', nameInput.trim()); 
                       setAppState('rules'); 
                     }
                   }}>
                      <input 
                        ...
                        className="w-full bg-[#121212] border border-white/10 rounded-2xl px-6 py-4 text-center text-lg text-white outline-none focus:border-blue-500/50 focus:bg-[#161616] transition-all mb-4" 
                      />
                      <button 
                        disabled={!nameInput.trim()} 
                        type="submit" 
                        className="w-full bg-white text-black font-semibold rounded-2xl px-6 py-4 hover:bg-zinc-200 transition-colors disabled:opacity-50"
                      >
                        Continue
                      </button>
                   </form>
      ...
      <AnimatePresence>
        {appState === 'rules' && (
          <motion.div 
            ...
            exit={{ opacity: 0, scale: 1.05 }} 
            transition={{ duration: 0.4, ease: smoothEase }} 
            className="fixed inset-0 z-[190] flex items-center justify-center bg-black"
          >
                ...
                <div className="space-y-4 mb-8">
                   <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                     ...
                     <p className="text-sm text-zinc-300">
                       Upload clear, text-selectable PDFs. Ask specific, analytical questions about the document structure. Use the model for high-speed local research.
                     </p>
                   </div>
                   <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                     <h3 className="font-semibold text-red-400 mb-2 flex items-center">
                       <AlertTriangle className="w-4 h-4 mr-2" /> DON'TS
                     </h3>
                     <p className="text-sm text-zinc-300">
                       Do not expect the model to browse the live internet. Do not upload encrypted or image-only PDFs without OCR. Do not refresh mid-generation.
                     </p>
                   </div>
                </div>
                <button 
                  onClick={() => setAppState('selecting')} 
                  className="w-full bg-blue-600 text-white font-semibold rounded-2xl px-6 py-4 hover:bg-blue-500 transition-colors"
                >
                  Acknowledge & Proceed
                </button>
             </div>
      ...
      {/* --- FLOW 3: MODEL SELECTION & SETTINGS --- */}
      <AnimatePresence>
            ...
            <div className="max-w-4xl w-full flex flex-col items-center relative">
              {showSettingsModal && (
                <button 
                  ...
                  className="absolute -top-12 right-0 p-2 text-zinc-500 hover:text-white rounded-full hover:bg-white/10 transition-colors"
                >
                  <X className="w-6 h-6" />
                </button>
              )}
              ...
              <motion.div initial={{ y: 20 }} animate={{ y: 0 }} className="text-center mb-12">
                <h1 className="text-4xl font-semibold tracking-tight mb-4">Neural Architecture</h1>
                <p className="text-zinc-400">Select an engine to power your local workspace.</p>
              </motion.div>
                ...
                {(Object.keys(AI_MODELS) as ModelTier[]).map((key) => {
                  ...
                  return (
                    <motion.button 
                      ...
                      onClick={() => { 
                        if (isCurrent) return; 
                        localStorage.setItem('axiom_selected_model', key); 
                        setShowSettingsModal(false); 
                        initWebLLM(key, false); 
                      }} 
                      className={`p-8 rounded-3xl border transition-all text-left group relative overflow-hidden ${isCurrent ? 'bg-blue-500/10 border-blue-500 cursor-default' : 'bg-white/5 border-white/10 hover:border-blue-500/50 hover:bg-white/10 cursor-pointer'}`}
                    >
                      <div className="absolute top-0 right-0 p-6 opacity-10 transition-opacity">
                        <Cpu className={`w-12 h-12 ${isCurrent ? 'text-blue-400 opacity-50' : 'text-white group-hover:opacity-100'}`} />
                      </div>
                      
                      <div className="flex items-center justify-between mb-2">
                          <h3 className={`text-2xl font-medium ${isCurrent ? 'text-blue-400' : 'text-white'}`}>
                            {AI_MODELS[key].name}
                          </h3>
                          {isCurrent && (
                            <span className="flex items-center px-3 py-1 bg-blue-500/20 border border-blue-500/30 text-blue-400 text-[10px] uppercase tracking-widest font-bold rounded-full">
                              <CheckCircle className="w-3 h-3 mr-1.5" /> Current
                            </span>
                          )}
                      </div>
                      ...
                      <div className="flex gap-4 relative z-10">
                        <span className="flex items-center text-xs font-mono text-zinc-500">
                          <Database className="w-3 h-3 mr-2" />{AI_MODELS[key].vram}
                        </span>
                        <span className="flex items-center text-xs font-mono text-zinc-500">
                          <ShieldCheck className="w-3 h-3 mr-2" />{AI_MODELS[key].minRam}
                        </span>
                      </div>
                    </motion.button>
                  );
      ...
      {/* --- FLOW 4: PREMIUM DOWNLOAD SCREEN --- */}
      <AnimatePresence>
        {appState === 'downloading' && (
          <motion.div 
            ...
            transition={{ duration: 0.6, ease: smoothEase }} 
            className="fixed inset-0 z-[110] flex items-center justify-center bg-[#030303] font-sans text-white"
          >
            <div className="absolute top-8 right-10 flex items-center space-x-3 text-zinc-500 text-xs tracking-[0.2em] font-medium uppercase">
            ...
            <div className="w-full max-w-2xl flex flex-col items-center">
              <h3 className="text-zinc-500 text-sm font-semibold mb-4 tracking-[0.3em] uppercase">
                Axiom-Zero
              </h3>
              
              <div className="h-10 mb-12 relative flex justify-center w-full overflow-hidden">
                  <AnimatePresence mode="wait">
                    <motion.h1 
                      key={getPhaseTitle(engineProgressPercent)} 
                      initial={{ opacity: 0, y: 15, filter: "blur(4px)" }} 
                      animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} 
                      exit={{ opacity: 0, y: -15, filter: "blur(4px)" }} 
                      transition={{ duration: 0.4, ease: "easeOut" }} 
                      className="text-white text-3xl font-medium tracking-tight absolute"
                    >
                      {getPhaseTitle(engineProgressPercent)}
                    </motion.h1>
                  </AnimatePresence>
              </div>
              
              <div className="w-full max-w-[400px] h-1.5 bg-white/10 rounded-full overflow-hidden mb-8 shadow-[0_0_15px_rgba(255,255,255,0.05)]">
                <motion.div 
                  className="h-full bg-white rounded-full" 
                  initial={{ width: 0 }} 
                  animate={{ width: `${engineProgressPercent}%` }} 
                  transition={{ ease: "linear", duration: 0.2 }} 
                />
              </div>
              
              <div className="w-full max-w-[400px] grid grid-cols-2 gap-4">
                 <div className="text-left flex flex-col space-y-1.5">
                   <span className="text-[9px] uppercase tracking-[0.2em] text-zinc-500 font-bold">Current Process</span>
                   <span className="text-xs font-mono text-zinc-300 truncate transition-opacity duration-300">
                     {engineProgressText || 'Awaiting stream...'}
                   </span>
                   <span className="text-xs font-mono text-zinc-400 mt-1">
                     <span className="text-zinc-600">SIZE:</span> {activeModelKey ? AI_MODELS[activeModelKey].vram : '--'}
                   </span>
                 </div>
                 <div className="text-right flex flex-col space-y-1.5 items-end">
                   <span className="text-[9px] uppercase tracking-[0.2em] text-zinc-500 font-bold">Progress</span>
                   <span className="text-xs font-mono text-zinc-300">{engineProgressPercent}%</span>
                   <span className="text-xs font-mono text-zinc-400 mt-1">
                     <span className="text-zinc-600">ETA:</span> {downloadTimeLeft}
                   </span>
                 </div>
              </div>
              
              <div className="absolute bottom-16 w-full max-w-xl text-center px-6 h-16 flex items-center justify-center">
                  <AnimatePresence mode="wait">
                    <motion.p 
                      key={factIndex} 
                      ...
                      className="text-[13px] text-zinc-500/80 leading-relaxed font-light"
                    >
             ...
             >
                 <div className="bg-[#121212] border border-white/10 rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
                     ...
                     <p className="text-sm text-zinc-400 mb-8">
                       This will permanently delete the vector embeddings and chat history for "{docToDelete.name}". This action cannot be undone.
                     </p>
                     <div className="flex space-x-3">
                       <button 
                         onClick={() => setDocToDelete(null)} 
                         className="flex-1 py-3 rounded-xl bg-white/5 hover:bg-white/10 font-semibold transition-colors"
                       >
                         Cancel
                       </button>
                       <button 
      ...
      <AnimatePresence>
        {isSidebarOpen && (
          <motion.div 
            ...
            className="h-full border-r border-white/5 bg-[#0a0a0a] flex flex-col flex-shrink-0 relative z-40"
          >
            <div className="h-16 flex items-center justify-between px-6 flex-shrink-0 min-w-[280px] border-b border-white/5">
              <span className="font-outfit text-[11px] font-bold tracking-[0.2em] uppercase text-zinc-500 truncate">
            ...
            <div className="p-4 min-w-[280px]">
               <button 
                 ...
                 className="w-full py-2.5 bg-blue-500/10 border border-blue-500/20 rounded-lg text-sm text-blue-400 font-medium hover:bg-blue-500/20 transition-colors flex items-center justify-center space-x-2"
               >
            ...
            <div className="flex-1 overflow-y-auto px-3 space-y-1 scrollbar-hide min-w-[280px] pb-4">
              <p className="text-[10px] uppercase tracking-wider text-zinc-600 mb-3 px-3 font-semibold mt-2">
                Recent Documents
              </p>
              ...
              {savedDocs.map(doc => (
                <div 
                  key={doc.id} 
                  className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg transition-all relative ${currentDocId === doc.id ? 'bg-white/10 text-white' : 'hover:bg-white/5 text-zinc-400'}`}
                >
                  <button 
                    onClick={() => handleLoadDocument(doc, userName)} 
                    className="flex-1 flex items-center space-x-3 overflow-hidden text-left outline-none"
                  >
                  ...
                  <button 
                    ...
                    className="p-1.5 text-zinc-500 hover:text-white rounded-md hover:bg-white/10 transition-colors"
                  >
                     ...
                     {activeDropdown === doc.id && (
                        <>
                           ...
                           <motion.div 
                             ...
                             className="absolute right-2 top-10 bg-[#161616] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden w-40"
                           >
                              <button 
                                onClick={(e) => { e.stopPropagation(); shareDocument(doc.id); }} 
                                className="w-full text-left px-4 py-2.5 text-xs text-zinc-300 hover:bg-white/5 flex items-center"
                              >
                              ...
                              <button 
                                ...
                                className="w-full text-left px-4 py-2.5 text-xs text-red-400 hover:bg-red-500/10 flex items-center"
                              >
            ...
            <div className="p-4 min-w-[280px] space-y-2 border-t border-white/5 bg-[#0a0a0a]">
              <button 
                ...
                className="w-full flex items-center justify-start space-x-3 text-sm text-zinc-400 hover:text-white px-3 py-2.5 rounded-lg transition-all hover:bg-white/5"
              >
              ...
              <button 
                ...
                className="w-full flex items-center justify-start space-x-3 text-sm text-red-400/70 hover:text-red-400 px-3 py-2.5 rounded-lg transition-all hover:bg-red-500/10"
              >
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 bg-[#050505] relative">
        <div className="h-16 flex items-center justify-between px-6 bg-[#050505] border-b border-white/5 shrink-0 z-20">
          <div className="flex items-center space-x-4">
            ...
            <span className="font-outfit text-lg font-semibold tracking-wide text-zinc-100">
              Axiom-Zero
            </span>
          </div>
          <div className="flex items-center space-x-4">
             <div className="hidden sm:flex items-center space-x-2 bg-white/5 border border-white/10 px-3 py-1.5 rounded-md">
               <Cpu className="w-3.5 h-3.5 text-zinc-400" />
               <span className="text-xs text-zinc-200 font-medium">
                 {activeModelKey ? AI_MODELS[activeModelKey].name : 'Engine'}
               </span>
             </div>
        ...
        {(appState === 'ready' || appState === 'initializing') ? (
                   ...
                   <div className="w-full max-w-md flex flex-col items-center">
                       ...
                       <div className="w-full h-1.5 bg-zinc-800 rounded-full overflow-hidden mb-2 shadow-[0_0_10px_rgba(59,130,246,0.2)]">
                           <motion.div 
                             ...
                             initial={{ width: "0%" }} 
                             animate={{ width: isVectorizingComplete ? "100%" : "95%" }} 
                             transition={{ duration: isVectorizingComplete ? 0.2 : 12, ease: "easeOut" }} 
                           />
                       </div>
                       <div className="w-full flex justify-between text-[10px] text-zinc-500 uppercase tracking-widest font-bold">
                         <span>Mapping Semantic Vectors</span>
                         <span className={isVectorizingComplete ? "text-blue-400" : "animate-pulse"}>
                           {isVectorizingComplete ? "Complete" : "Processing..."}
                         </span>
                       </div>
                   </div>
               ) : ( 
           ...
           <div className="flex-1 flex flex-row overflow-hidden relative w-full">
              <div 
                className="hidden lg:flex h-full border-r border-white/5 flex-col bg-[#0a0a0a] shrink-0 relative" 
              ...
              >
                 ...
                 />
                 <div 
                   className="absolute top-0 right-[-4px] bottom-0 w-2 cursor-col-resize hover:bg-blue-500/50 active:bg-blue-500 transition-colors z-20 flex items-center justify-center group" 
              ...
              <div className="flex-1 flex flex-col h-full min-w-0 bg-[#050505] relative z-10">
                        ...
                        {messages.map((msg) => (
                           ...
                           >
                              <div className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse max-w-[85%]' : 'w-full min-w-0'}`}>
                                 ...
                                 {msg.role === 'user' && (
                                   <div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center mt-1 flex-shrink-0">
                                 ...
                                 <div className="flex flex-col w-full min-w-0 group relative">
                                    {msg.role === 'user' ? (
                                      <>
                                        <div className="px-5 py-3.5 rounded-2xl bg-[#1e1e1e] border border-white/5 text-zinc-100 text-[15px] leading-relaxed whitespace-pre-wrap">
                                          {msg.content}
                                        </div>
                                      ...
                                      <>
                                        ...
                                        <div className="flex items-center gap-1 mt-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                           <button 
                                             onClick={() => copyToClipboard(msg.content, msg.id)} 
                                             className="p-1.5 rounded-lg border border-transparent hover:bg-white/5 text-zinc-500 hover:text-white transition-all flex items-center space-x-1.5"
                                           >
                                           ...
                                           {!isTyping && (
                                             <button 
                                               ...
                                               className="p-1.5 rounded-lg border border-transparent hover:bg-white/5 text-zinc-500 hover:text-white transition-all"
                                             >
                                           ...
                                           <button 
                                             onClick={() => setFeedbackState(p => ({...p, [msg.id]: 'like'}))} 
                                             className={`p-1.5 rounded-lg border border-transparent transition-all ${feedbackState[msg.id] === 'like' ? 'text-emerald-400 bg-emerald-500/10' : 'text-zinc-500 hover:bg-white/5 hover:text-emerald-400'}`}
                                           >
                                             <ThumbsUp className="w-3.5 h-3.5" />
                                           </button>
                                           <button 
                                             onClick={() => setFeedbackState(p => ({...p, [msg.id]: 'dislike'}))} 
                                             className={`p-1.5 rounded-lg border border-transparent transition-all ${feedbackState[msg.id] === 'dislike' ? 'text-red-400 bg-red-500/10' : 'text-zinc-500 hover:bg-white/5 hover:text-red-400'}`}
                                           >
                                             <ThumbsDown className="w-3.5 h-3.5 mt-1" />
                                           </button>
                                        </div>
                                        {msg.sources && msg.sources.length > 0 && (
                                           <details className="mt-4 border-t border-white/5 pt-3">
                                               <summary className="text-xs text-zinc-500 font-medium cursor-pointer hover:text-zinc-300 transition-colors select-none flex items-center">
                                                 <ArrowRight className="w-3 h-3 mr-1 inline transform transition-transform group-open:rotate-90" /> 
                                                 Data Provenance ({msg.sources.length})
                                               </summary>
                                              <div className="mt-3 grid grid-cols-1 gap-3 border-l-2 border-white/10 pl-4 ml-1">
                                                 {msg.sources.map((src, i) => (
                                                    <div key={`source-${i}`} className="flex flex-col space-y-1">
                                                      <button 
                                                        onClick={() => pdfViewerRef.current?.setPage(src.pageNumber)} 
                                                        className="w-fit text-[10px] font-mono text-zinc-400 bg-white/5 px-2 py-0.5 rounded uppercase font-semibold hover:bg-white/10 transition-colors border border-white/10"
                                                      >
                                                        Page {src.pageNumber}
                                                      </button>
                 ...
                 <div className="w-full p-4 lg:p-6 bg-gradient-to-t from-[#050505] via-[#050505] to-transparent flex flex-col items-center shrink-0 z-20 relative">
                    <form 
                      ...
                      className="w-full max-w-3xl relative flex items-center bg-[#161616] border border-white/10 rounded-2xl p-1.5 shadow-2xl focus-within:border-blue-500/40 focus-within:bg-[#1a1a1a] transition-all"
                    >
                       <input 
                         ref={inputRef} 
                         type="text" 
                         ...
                         disabled={isTyping || !engineReady} 
                         placeholder={isTyping ? "Synthesizing response..." : "Ask Axiom-Zero..."} 
                         className="flex-1 bg-transparent text-white text-[15px] py-3.5 px-4 outline-none disabled:opacity-50 placeholder:text-zinc-600 font-open-sans" 
                       />
                       {isTyping ? (
                           <button 
                             type="button" 
                             onClick={stopGeneration} 
                             className="p-3 mr-1 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all border border-red-500/20"
                           >
                           ...
                           <button 
                             type="submit" 
                             disabled={!searchQuery.trim() || !engineReady} 
                             className="p-3 mr-1 rounded-xl bg-blue-500 text-white hover:bg-blue-400 transition-all disabled:opacity-50 disabled:bg-[#222] disabled:text-zinc-600 shadow-md flex items-center justify-center"
                           >
