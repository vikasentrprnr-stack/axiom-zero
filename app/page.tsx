...
import { 
  ...
  ThumbsDown, Info, MonitorSmartphone, ShieldCheck, HelpCircle, 
  MoreVertical, Share2, Download, CheckCircle
} from 'lucide-react';
...
const LOADING_FACTS = [
  "Axiom-Zero operates entirely within your local hardware architecture. By leveraging WebGPU, Neural Networks compile and execute directly on your silicon, guaranteeing absolute data sovereignty with zero cloud transmission.",
  "Large Language Models (LLMs) operate in high-dimensional vector spaces. They predict the next logical token by analyzing billions of mathematical parameters simultaneously, effectively synthesizing human-like reasoning.",
  "Web Workers isolate complex tensor operations from the main browser thread. This multi-threaded architecture ensures your interface remains at a flawless 60 frames per second, even while allocating gigabytes of VRAM.",
  "We utilize advanced quantization techniques (q4f16) to compress massive neural weights. This reduces the memory footprint by up to 400% while retaining elite-level cognitive and analytical capabilities.",
  "During document ingestion, text is transformed into dense semantic embeddings. Axiom-Zero maps these concepts into a 3D mathematical space, allowing it to instantly retrieve exact context matches based on conceptual similarity."
];

// FIX 1: Explicit tuple typing to satisfy Framer Motion Easing requirements
...
export default function Home() {
    ...
    const checkEnvironment = async () => {
      ...
      try {
        ...
        if (adapter && ((navigator as any).deviceMemory > 8 || adapter.limits.maxBufferSize >= 2147483648)) {
          ...
          console.warn("Hardware tier check fallback triggered.");
      }
      ...
      if (lastDocId) {
         ...
         if (targetDoc) {
             handleLoadDocument(targetDoc, savedName);
         } else {
             setAppState('ready');
         }
      } else {
  ...
  // === 2. MULTI-THREADED ENGINE INITIALIZATION ===
    ...
    try {
      ...
      if (!isSilentLoad) {
         ...
         if (lastDocId) {
             ...
             const targetDoc = docs.find(d => d.id === lastDocId);
             if (targetDoc) { 
                 handleLoadDocument(targetDoc, userName); 
                 return; 
             }
         }
         setAppState('ready');
      ...
      if (err.message?.includes('Cache') || err.name === 'NetworkError') {
          alert(`Storage Error: Browser failed to save model weights. Wiping corrupted cache.`);
  ...
  useEffect(() => {
    ...
    embedWorker.current.onmessage = async (e: MessageEvent) => {
      ...
      if (type === 'EMBED_COMPLETE') {
          ...
          setTimeout(() => setAppState('active'), 300); 
      }
      ...
      if (type === 'QUERY_EMBEDDED') {
          ...
          if (contextText.length > 2500) {
              contextText = contextText.substring(0, 2500) + "... [Truncated]"; 
          }
          ...
          try {
            ...
            const systemPrompt = `You are Axiom-Zero, a highly intelligent local AI. The user is asking a question. Context from their document is provided below. IF the context contains the answer, use it to form a highly structured, accurate response. IF the user is asking a general knowledge question that is NOT in the context, answer it intelligently using your own internal knowledge. Be concise, fast, and structured.`;
            ...
            setMessages(prev => [...prev, { id: `ai-${Date.now()}`, role: 'assistant', content: fullResponse, sources: finalChunks } as ChatMessage]);
             ...
             if (err.name !== 'AbortError') {
                 setMessages(prev => [...prev, { id: `err`, role: 'assistant', content: `System Error: ${err.message}` } as ChatMessage]);
             }
          } finally {
             setStreamingContent(''); 
             setTypingStatus(''); 
  ...
  // === UI HANDLERS ===
  const handleLoadDocument = useCallback(async (doc: any, name: string) => {
    ...
    try {
        ...
        if (doc.messages && doc.messages.length > 0) {
            ...
            setMessages([{ id: `w-${Date.now()}`, role: 'assistant', content: `Hi **${name}**, how can I help you with **${doc.name}** today?` } as ChatMessage]);
        }
    } catch (e) { 
  ...
  const handleIngestionComplete = async (chunks: TextChunk[], file: File) => {
    ...
    try {
      ...
      setCurrentDocName(chunks[0].documentName);
      ...
      setMessages([{ id: `w-${Date.now()}`, role: 'assistant', content: `Hi **${userName}**, how can I help you with **${chunks[0].documentName}** today?` } as ChatMessage]);
    } catch (e) { 
  ...
  // FIX 2: Restored missing `copyToClipboard` function exactly where it needs to be
  ...
  const shareDocument = (docId: string) => {
      navigator.clipboard.writeText(`${window.location.origin}/shared/${docId}`);
      alert("Share link copied to clipboard! (Note: Localhost links only work on this network)");
  ...
  const exportDocument = (doc: any) => {
      ...
      let content = `# Axiom-Zero Report: ${doc.name}\nGenerated for ${userName}\n\n`;
      doc.messages.forEach((m: any) => { 
          content += `**${m.role === 'user' ? userName : 'Axiom-Zero'}**:\n${m.content}\n\n---\n\n`; 
      });
      
      const blob = new Blob([content], { type: 'text/markdown;charset=utf-8' });
      const a = document.createElement('a'); 
      a.href = URL.createObjectURL(blob); 
      a.download = `Axiom_${doc.name}.md`;
  ...
  const confirmDelete = async () => {
      ...
      if (currentDocId === docToDelete.id) {
          clearWorkspace();
      }
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    const handleMouseMove = (ev: MouseEvent) => {
       ...
       animId = requestAnimationFrame(() => {
           let newWidth = (ev.clientX / window.innerWidth) * 100;
           setLeftWidth(Math.max(25, Math.min(newWidth, 60)));
       });
    };
  ...
  if (isMobile) return (
      <div className="flex h-screen w-screen bg-[#050505] text-white items-center justify-center p-6 font-sans">
          <div className="max-w-md text-center">
              <MonitorSmartphone className="w-16 h-16 text-blue-500 mx-auto opacity-80 mb-6" />
              <h2 className="text-3xl font-semibold mb-2">Desktop Required</h2>
              <p className="text-zinc-400">Axiom-Zero requires desktop WebGPU memory to function safely.</p>
          </div>
      </div>
  );
  
  if (browserSupported === false) return (
      <div className="flex h-screen w-screen bg-[#050505] text-white items-center justify-center p-6">
          <div className="bg-[#121212] border border-white/10 p-8 rounded-2xl max-w-lg w-full text-center">
              <AlertTriangle className="w-12 h-12 text-red-500 mx-auto mb-4" />
              <h2 className="text-2xl font-semibold mb-3">Unsupported Browser</h2>
              <p className="text-zinc-400">Safari and older browsers are unsupported by WebGPU.</p>
          </div>
      </div>
  );
  ...
  return (
    <div className="flex h-screen w-screen bg-[#050505] text-white font-open-sans overflow-hidden">
             ...
             <div className="max-w-md w-full text-center p-6">
                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1, duration: 0.6, ease: smoothEase }}>
                   ...
                   <form onSubmit={(e) => { 
                      ...
                      />
                      <button disabled={!nameInput.trim()} type="submit" className="w-full bg-white text-black font-semibold rounded-2xl px-6 py-4 hover:bg-zinc-200 transition-colors disabled:opacity-50">
                          Continue
                      </button>
                   </form>
                </motion.div>
             ...
             <div className="max-w-2xl w-full p-8 bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-2xl">
                ...
                <div className="space-y-4 mb-8">
                   <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                      <h3 className="font-semibold text-emerald-400 mb-2 flex items-center"><CheckCircle className="w-4 h-4 mr-2" /> DO'S</h3>
                      <p className="text-sm text-zinc-300">Upload clear, text-selectable PDFs. Ask specific, analytical questions about the document structure. Use the model for high-speed local research.</p>
                   </div>
                   <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                      <h3 className="font-semibold text-red-400 mb-2 flex items-center"><AlertTriangle className="w-4 h-4 mr-2" /> DON'TS</h3>
                      <p className="text-sm text-zinc-300">Do not expect the model to browse the live internet. Do not upload encrypted or image-only PDFs without OCR. Do not refresh mid-generation.</p>
                   </div>
                </div>
                <button onClick={() => setAppState('selecting')} className="w-full bg-blue-600 text-white font-semibold rounded-2xl px-6 py-4 hover:bg-blue-500 transition-colors">
                    Acknowledge & Proceed
                </button>
             </div>
          </motion.div>
      ...
      {/* --- FLOW 3: MODEL SELECTION & SETTINGS (FIX 3: Current Model State Disabled) --- */}
      <AnimatePresence>
        {(appState === 'selecting' || showSettingsModal) && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 1.05 }} transition={{ duration: 0.4, ease: smoothEase }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-2xl p-6">
            <div className="max-w-4xl w-full flex flex-col items-center relative">
              ...
              {showSettingsModal && (
                 <button onClick={() => setShowSettingsModal(false)} className="absolute -top-12 right-0 p-2 text-zinc-500 hover:text-white rounded-full hover:bg-white/10 transition-colors">
                    <X className="w-6 h-6" />
                 </button>
              )}
                ...
                {(Object.keys(AI_MODELS) as ModelTier[]).map((key) => {
                  ...
                  return (
                    <motion.button 
                      ...
                      onClick={() => {
                         if (isCurrent) return; // Prevent selecting the already active model
                      ...
                      className={`p-8 rounded-3xl border transition-all text-left group relative overflow-hidden ${
                        isCurrent 
                          ? 'bg-blue-500/10 border-blue-500 cursor-default' 
                          : 'bg-white/5 border-white/10 hover:border-blue-500/50 hover:bg-white/10 cursor-pointer'
                      }`}
                    >
                      ...
                      <div className="flex items-center justify-between mb-2">
                          <h3 className={`text-2xl font-medium ${isCurrent ? 'text-blue-400' : 'text-white'}`}>{AI_MODELS[key].name}</h3>
                          {isCurrent && (
                      ...
                      <div className="flex gap-4 relative z-10">
                        <span className="flex items-center text-xs font-mono text-zinc-500"><Database className="w-3 h-3 mr-2" />{AI_MODELS[key].vram}</span>
                        <span className="flex items-center text-xs font-mono text-zinc-500"><ShieldCheck className="w-3 h-3 mr-2" />{AI_MODELS[key].minRam}</span>
                      </div>
                    </motion.button>
                  );
                })}
             ...
             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[300] flex items-center justify-center bg-black/80 backdrop-blur-sm p-6">
                 <div className="bg-[#121212] border border-white/10 rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
                     ...
                     <div className="flex space-x-3">
                         <button onClick={() => setDocToDelete(null)} className="flex-1 py-3 rounded-xl bg-white/5 hover:bg-white/10 font-semibold transition-colors">Cancel</button>
                         <button onClick={confirmDelete} className="flex-1 py-3 rounded-xl bg-red-600 text-white font-semibold hover:bg-red-500 transition-colors">Delete</button>
                     </div>
                 </div>
             </motion.div>
         )}
        ...
        {isSidebarOpen && (
          ...
          >
            <div className="h-16 flex items-center justify-between px-6 flex-shrink-0 min-w-[280px] border-b border-white/5">
              ...
              <button onClick={toggleSidebar} className="p-1.5 rounded-md text-zinc-500 hover:text-white hover:bg-white/5 transition-colors">
                  <X className="w-4 h-4" />
              </button>
            </div>
            
            <div className="p-4 min-w-[280px]">
               <button onClick={clearWorkspace} className="w-full py-2.5 bg-blue-500/10 border border-blue-500/20 rounded-lg text-sm text-blue-400 font-medium hover:bg-blue-500/20 transition-colors flex items-center justify-center space-x-2">
            ...
            <div className="flex-1 overflow-y-auto px-3 space-y-1 scrollbar-hide min-w-[280px] pb-4">
              ...
              {savedDocs.map(doc => (
                <div key={doc.id} className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg transition-all relative ${currentDocId === doc.id ? 'bg-white/10 text-white' : 'hover:bg-white/5 text-zinc-400'}`}>
                  <button onClick={() => handleLoadDocument(doc, userName)} className="flex-1 flex items-center space-x-3 overflow-hidden text-left outline-none">
                    <FileText className="w-4 h-4 flex-shrink-0 text-zinc-500" />
                    <span className="text-[13px] truncate font-medium">{doc.name}</span>
                  </button>
                  
                  <button onClick={(e) => { e.stopPropagation(); setActiveDropdown(activeDropdown === doc.id ? null : doc.id); }} className="p-1.5 text-zinc-500 hover:text-white rounded-md hover:bg-white/10 transition-colors">
            ...
            <div className="p-4 min-w-[280px] space-y-2 border-t border-white/5 bg-[#0a0a0a]">
              <button onClick={() => window.open('https://github.com/mlc-ai/web-llm', '_blank')} className="w-full flex items-center justify-start space-x-3 text-sm text-zinc-400 hover:text-white px-3 py-2.5 rounded-lg transition-all hover:bg-white/5">
                  <HelpCircle className="w-4 h-4 text-zinc-500" /><span>Help & Documentation</span>
              </button>
              <button onClick={() => setShowSettingsModal(true)} className="w-full flex items-center justify-start space-x-3 text-sm text-zinc-400 hover:text-white px-3 py-2.5 rounded-lg transition-all hover:bg-white/5">
                  <Settings2 className="w-4 h-4 text-zinc-500" /><span>Change Model</span>
              </button>
              <button onClick={async () => { await clearDB(); setSavedDocs([]); clearWorkspace(); setAppState('selecting'); }} className="w-full flex items-center justify-start space-x-3 text-sm text-red-400/70 hover:text-red-400 px-3 py-2.5 rounded-lg transition-all hover:bg-red-500/10">
        ...
        <div className="h-16 flex items-center justify-between px-6 bg-[#050505] border-b border-white/5 shrink-0 z-20">
          ...
          <div className="flex items-center space-x-4">
             <div className="hidden sm:flex items-center space-x-2 bg-white/5 border border-white/10 px-3 py-1.5 rounded-md">
                 ...
                 <span className="text-xs text-zinc-200 font-medium">{activeModelKey ? AI_MODELS[activeModelKey].name : 'Engine'}</span>
             </div>
             <div className={`flex items-center space-x-2 px-3 py-1.5 rounded-md border transition-colors duration-300 ${!engineReady ? 'border-amber-500/20 bg-amber-500/10 text-amber-400' : 'border-emerald-500/20 bg-emerald-500/10 text-emerald-400'}`}>
                ...
                <span className="text-[10px] font-bold uppercase tracking-wider hidden sm:inline whitespace-nowrap">{engineReady ? 'Active' : 'Allocating'}</span>
             </div>
          </div>
        </div>
           ...
           <div className="flex-1 flex flex-col items-center justify-center p-6">
               {appState === 'initializing' ? (
                   <div className="flex flex-col items-center space-y-4">
                       ...
                       <p className="text-lg font-medium text-zinc-200">Vectorizing Document...</p>
                   </div>
               ) : (
              ...
              <div className="flex-1 flex flex-col h-full min-w-0 bg-[#050505] relative z-10">
                                        ...
                                        {msg.sources && msg.sources.length > 0 && (
                                           <details className="mt-4 border-t border-white/5 pt-3">
                                               <summary className="text-xs text-zinc-500 font-medium cursor-pointer hover:text-zinc-300 transition-colors select-none flex items-center">
                                                   <ArrowRight className="w-3 h-3 mr-1 inline transform transition-transform group-open:rotate-90" /> Data Provenance ({msg.sources.length})
                                               </summary>
                 ...
                 <div className="w-full p-4 lg:p-6 bg-gradient-to-t from-[#050505] via-[#050505] to-transparent flex flex-col items-center shrink-0 z-20 relative">
                    <form onSubmit={handleSearch} className="w-full max-w-3xl relative flex items-center bg-[#161616] border border-white/10 rounded-2xl p-1.5 shadow-2xl focus-within:border-blue-500/40 focus-within:bg-[#1a1a1a] transition-all">
                       ...
                       />
                       {isTyping ? (
                           <button type="button" onClick={stopGeneration} className="p-3 mr-1 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all border border-red-500/20">
                               <Square className="w-4 h-4 fill-current" />
                           </button>
                       ) : (
                           <button type="submit" disabled={!searchQuery.trim() || !engineReady} className="p-3 mr-1 rounded-xl bg-blue-500 text-white hover:bg-blue-400 transition-all disabled:opacity-50 disabled:bg-[#222] disabled:text-zinc-600 shadow-md flex items-center justify-center">
